...
def load_raw(path: Path) -> tuple[VGGTResult, SceneNormalization, np.ndarray | None]:
    ...
    with np.load(path, allow_pickle=True) as data:
        ...
        if {"scene_center", "scene_scale"} <= keys:
            ...
            normalization = compute_normalization(points[np.isfinite(points).all(axis=-1)])

        masks = None
        if {"masks_packed", "masks_shape"} <= keys:
            shape = tuple(int(v) for v in data["masks_shape"])
            count = int(np.prod(shape))
            masks = np.unpackbits(data["masks_packed"])[:count].astype(bool).reshape(shape)
...
def remesh(
    ...
    if backend == "tsdf":
        mesh = mesh_tsdf.fuse(
            ...
            config=mesh_tsdf.TSDFConfig(voxel_size=voxel_size), masks=masks, device=device, log_fn=log_fn,
        )
    ...
    mesh = cleanup_module.clean(
        ...
        bbox_min, bbox_max, None, voxel_size, log_fn,
    )
    metrics = cleanup_module.mesh_metrics(mesh)

    scale = 1.0
    if ref_size:
        vertices = np.asarray(mesh.vertices)
        extent = float((vertices.max(axis=0) - vertices.min(axis=0)).max())
        scale = float(ref_size) / extent if extent > 1e-9 else 1.0
